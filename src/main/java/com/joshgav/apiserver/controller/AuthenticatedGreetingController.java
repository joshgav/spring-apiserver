...
@RestController
@RequestMapping(value={"/auth/greeting"})
public class AuthenticatedGreetingController {
    private static final Logger logger = LoggerFactory.getLogger(AuthenticatedGreetingController.class);

    @Operation(summary = "greet authenticated user", tags = {"profile"})
    @GetMapping(path="", produces = MediaType.TEXT_PLAIN_VALUE)
    public ResponseEntity<String> greet(HttpServletRequest req) {
        logger.info("authenticated greeting requested");
